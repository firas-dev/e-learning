...
import {
  ...
  Clock, Mail, Loader2, ChevronDown, ChevronUp,
  Bell,
} from 'lucide-react';
...
function InvitationCard({
  ...
  const teacherInitials = teacher?.fullName
    ?.split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2) || 'T';
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {invitation.roomDescription && (
              <p className="text-sm text-gray-500 mb-3">{invitation.roomDescription}</p>
            )}
            ...
            <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl mb-4">
              ...
              <div className="ml-auto text-right">
                <p className="text-xs text-gray-400">
                  {new Date(invitation.invitedAt).toLocaleDateString(undefined, {
                    month: 'short',
                    day: 'numeric',
                    year: 'numeric',
                  })}
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 py-2.5 border-2 border-gray-200 text-gray-700 hover:border-red-300 hover:text-red-600 hover:bg-red-50 rounded-xl font-medium text-sm transition-all disabled:opacity-50"
              >
                {responding === 'decline' ? (
              ...
              <button
                ...
                disabled={responding !== null}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-violet-600 hover:bg-violet-700 text-white rounded-xl font-medium text-sm transition-colors disabled:opacity-50 shadow-sm"
              >
                {responding === 'accept' ? (
                  <Loader2 className="w-4 h-4 animate-spin" />
                ) : (
...
function RoomCard({ room }: { room: PrivateRoom }) {
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">Your exclusive learning spaces</p>
          </div>
        </div>
          ...
          <>
            ...
            {invitations.length > 0 && (
              <section className="mb-8">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="w-6 h-6 bg-violet-600 text-white text-xs font-bold rounded-full flex items-center justify-center">
                    {invitations.length}
                  </span>
                </div>
            ...
            <section>
              <h2 className="text-xl font-bold text-gray-900 mb-4">
                My Rooms
                {rooms.length > 0 && (
                  <span className="ml-2 text-sm font-normal text-gray-500">({rooms.length})</span>
                )}
              </h2>
              ...
              {rooms.length === 0 && invitations.length === 0 ? (
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-16 text-center">
                  ...
                  <p className="text-gray-500 text-sm max-w-sm mx-auto">
                    When a teacher invites you to a private room, you'll see the invitation here. Check your notifications!
                  </p>
                </div>
              ) : rooms.length === 0 ? (
                <div className="bg-white rounded-2xl border border-dashed border-gray-300 p-10 text-center">
                  ...
                  <p className="text-gray-400 text-sm">
                    Accept an invitation above to join your first private room.
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
